...
export function AuthDialog({ open, onOpenChange }: AuthDialogProps) {
  ...
  const handleSubmit = async (mode: "signin" | "signup") => {
    ...
    if (error) { setError(error.message); }
    else { setEmail(""); setPassword(""); setConfirmPassword(""); onOpenChange(false); }
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        ...
        <div className="relative mb-2">
          ...
          <div className="relative flex justify-center text-xs uppercase"><span className="bg-background px-2 text-muted-foreground">{t("auth.orEmail")}</span></div>
        </div>
        ...
        <Tabs defaultValue="signin" className="w-full">
          ...
          <TabsContent value="signup" className="space-y-4 mt-4">
            ...
            <div className="space-y-2">
              ...
              <Input id="signup-confirm-password" type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="••••••••" />
            </div>
            {error && <p className="text-sm text-destructive">{error}</p>}
